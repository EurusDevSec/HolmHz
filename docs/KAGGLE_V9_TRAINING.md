# Kaggle Training Guide — v9 (JPEG Aug Fix + CLIP Detector)

Hướng dẫn train v9 trên Kaggle T4 GPU.
2 hướng fix iPhone/Social Media false positive:
- **Hướng A**: EfficientNet-B0 + JPEG augmentation v3 (cùng dataset v8)
- **Hướng B**: CLIP ViT-L/14 Linear Probe (cùng dataset v8)
...
```bash
# Đã tạo sẵn:
# holmhz-code-v9.zip  (0.4 MB)  — src, configs, scripts, manifests
# holmhz-images-v8.zip (1.6 GB) — tất cả ảnh (GIỐNG v8, không cần upload lại nếu đã có)
...
# === LINK/COPY IMAGES ===
# Tạo symlinks cho TẤT CẢ thư mục ảnh
...
### Cell 5: Write v9 Config

```python
# ══════════════════════════════════════════════════
# CHỌN 1 TRONG 2:
# - Hướng A: EfficientNet + JPEG Aug (recommend chạy trước)
# - Hướng B: CLIP Linear Probe (chạy sau nếu cần)
# ══════════════════════════════════════════════════

# ── Hướng A: EfficientNet-B0 + JPEG Augmentation v3 ──
config_v9 = """
model:
...
with open("configs/train_v9.yaml", "w") as f:
    f.write(config_v9)
print("✅ configs/train_v9.yaml written (EfficientNet + JPEG Aug v3)")

# ── Hướng B: CLIP ViT-L/14 Linear Probe (uncomment nếu muốn chạy) ──
# ⚠️ Cần: !pip install -q open-clip-torch
#
# config_v9_clip = """
# model:
#   name: clip_vit_l14
...
#   log_every_n_steps: 10
# """
# with open("configs/train_v9_clip.yaml", "w") as f:
#     f.write(config_v9_clip)
# print("✅ configs/train_v9_clip.yaml written (CLIP Linear Probe)")
```
...
# ── Hướng A: EfficientNet + JPEG Aug v3 ──
print("Training EfficientNet-B0 v9 (JPEG augmentation fix)...")
!PYTHONPATH=src python scripts/train.py configs/train_v9.yaml data.num_workers=4

# ── Hướng B: CLIP (uncomment nếu muốn chạy) ──
# !pip install -q open-clip-torch
# print("Training CLIP ViT-L/14 Linear Probe...")
# !PYTHONPATH=src python scripts/train.py configs/train_v9_clip.yaml data.num_workers=4
```
...
if best.exists():
    shutil.copy2(best, "/kaggle/working/best_v9.pt")
    print(f"✅ Saved /kaggle/working/best_v9.pt ({best.stat().st_size/1e6:.1f}MB)")
else:
...
evaluation:
  ...
  output_dir: outputs/evaluation_v9
...
with open("configs/test_v9.yaml", "w") as f:
    f.write(test_config)
print("✅ configs/test_v9.yaml written")

# Full evaluation
print("\n=== Full Evaluation (ID + OOD) ===")
!PYTHONPATH=src python scripts/test.py configs/test_v9.yaml
```

---

## Thay đổi v9 so với v8

### Hướng A — JPEG Augmentation (trong code, không cần config thay đổi)
- `transforms.py` v3: JPEG compression p=0.7 (quality 50-95) — dedicated, không nằm trong OneOf
- Social media resize simulation: Downscale p=0.3 (scale 50-90%)
- Fix: iPhone HEIC→JPEG, Facebook q=80-85, Instagram q=85-90

### Hướng B — CLIP Detector (uncomment trong Cell 5+6 để chạy)
- CLIP ViT-L/14 frozen + Linear(768→1) — chỉ 769 trainable params
- Cần `!pip install -q open-clip-torch` (thêm ~1GB)
- Train nhanh: ~15-30 phút (vs 60-90 phút cho EfficientNet)

## Estimates

| Metric | Hướng A (EfficientNet) | Hướng B (CLIP) |
|--------|:-:|:-:|
| Training time | ~60-90 phút | ~15-30 phút |
| VRAM | ~4GB | ~8GB |
| Upload size | code 0.4MB + images 1.6GB | + open-clip 1GB |
| Trainable params | 4M | 769 |
| Target | Fix JPEG bias | Fix domain generalization |
